...
try:
    ...
    print(
        "❌ Error: atpack_parser module not found. Please ensure atpack-python-parser is available."
    )
    sys.exit(1)
...
class BoardGenerator:
                ...
                "tools": {
                    "mplab-ice": {
                        "server": {
                            ...
                            "arguments": ["--chip", device_name.upper()],
                        }
                    }
                }
            },
        ...
        board_config = {
            ...
            "upload": {
                ...
                "info": {
                    "DeviceID": self.get_real_device_id(device_name),
                    "FlashEnd": flash_end,
                    "Eeprom": spec.get("eeprom_addr", "0x0")
                    if spec.get("eeprom_addr")
                    else "0x0",
                    "EepromSize": spec.get("eeprom_size", 0),
                    "Config": spec.get("config_addr", "0x0")
                    if spec.get("config_addr")
                    else "0x0",
                    "ConfigSize": spec.get("config_size", 0),
                },
                "tool": {
    ...
    def generate_from_atpack(
        self,
        atpack_path: str,
        device_filter: Optional[List[str]] = None,
        pic16f_only: bool = False,
    ) -> List[Path]:
        ...
        try:
            ...
            if pic16f_only:
                all_specs = [
                    ...
                    if spec.device_name.upper().startswith("PIC16F")
                ]
            ...
            if device_filter:
                filtered_specs = [
                    spec for spec in all_specs if spec.device_name in device_filter
                ]
...
Examples:
  ...
  python create_boards.py ../../../atpack-python-parser/atpacks/Microchip.PIC16Fxxx_DFP.1.7.162.atpack --output-dir /custom/boards/dir
    ...
    parser.add_argument("atpack_path", help="Path to the AtPack file to process")
    ...
    try:
        ...
        generated = generator.generate_from_atpack(
            str(atpack_path), device_filter, pic16f_only=args.pic16f_only
        )
